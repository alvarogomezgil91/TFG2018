...
if [[ -z $crumb ]]; then
	...
	if [[ -z $crumb ]]; then
		...
		echo $1 >> "${FilePath}/error_${today}.csv"
		echo "./get-yahoo-quotes-today.sh $1" >> "${FilePath}/error_aux_${today}.csv"
		echo "./genera_query_today.sh $1" >> "${FilePath}/error_aux_${today}.csv"
		exit 1
	fi
fi
...
# Download to 
curl -s --cookie $cookieJar  $URL > "${FilePath}/${SYMBOL}.aux"

awk 'NR > 1' "${FilePath}/${SYMBOL}.aux" > "${FilePath}/${SYMBOL}.csv"
rm -f "${FilePath}/${SYMBOL}.aux"
